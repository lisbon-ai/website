...
canvas) and `[data-motifs-toggle]` (an initially hidden button). Preserve the
...
website uses top-centered `cover`, matching its original responsive video layout.
...
autoplay and initial GPU/filter allocation. The button permits explicit play.
