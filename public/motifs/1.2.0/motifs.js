...
n.map((e) => e === "rosenblatt" ? "deep-blue" : e);
//#endregion
...
function i(e = r) {
	...
	let t = [], n = /* @__PURE__ */ new Map(), i = s(0, !0, -Infinity, Infinity), o = [0];
	for (; n.has(o.at(-1));) o.push(n.get(o.at(-1)));
	return {
		value: i,
		path: o,
		events: t
	...
	function s(r, i, o, c) {
		...
		for (let [e, d] of u.entries()) {
			let f = s(d, !i, o, c);
			if ((i ? f > l : f < l) && (l = f, n.set(r, d)), i ? o = Math.max(o, l) : c = Math.min(c, l), o >= c && e === 0) {
				t.push({
					kind: "cutoff",
					node: r,
					skipped: a(u[1]),
...
var o = [
	...
	1.38,
	1.87
], g = (e, t = 0, n = 1) => Math.max(t, Math.min(n, e)), _ = (e, t, n) => e + (t - e) * n, v = (e, t, n) => g((e - t) / (n - t)), y = (e) => {
	let t = g(e);
	return t * t * (3 - 2 * t);
}, b = (e) => (e % 20 + 20) % 20, x = (e) => b(e) * f;
function S(e, t) {
	let n = x(e);
	return n < 8.3 ? 8.3 * v(n, h[t], 8.3) : n;
}
function C(e, t) {
	let n = (x(e) - [
		.05,
	...
	return y(v(n, 0, .28)) * (1 - y(v(n, .755, 2.085))) - y(v(n, 5.6, 6.55)) * (1 - y(v(n, 7.25, 8.3)));
}
function w(e, t = 17) {
...
function D(e) {
	...
	for (let e = 0; e < t; e++) n.push(E[e % E.length]);
	return {
		tape: n,
		head: t,
		state: t % E.length
...
var k = "10110110110110101110";
function A(e) {
	let t = [...e].filter((e) => e === "1").length, n = t + 1, r = e.length - t + 1;
	return [j(.025, n, r), j(.975, n, r)];
}
function j(e, t, n) {
	let r = 0, i = 1;
	for (let a = 0; a < 48; a++) {
...
function P(e, t = n) {
	...
	r.width = r.height = 960;
	let a = r.getContext("2d"), o = pe(e), s, c = {
		bayes: I,
		lovelace: z,
		turing: B,
		shannon: te,
		...
		alphago: ce,
		transformer: de,
		"deep-blue": (e, t) => re(e, t, s ??= i())
	};
	return {
		canvas: r,
		draw(e) {
			a.fillStyle = "black", a.fillRect(0, 0, r.width, r.height), t.forEach((t, n) => {
				a.save(), a.translate(n % 3 * 320, Math.floor(n / 3) * 320), a.scale(320, 320), X(a, .005, .005, .99, .99, .045), a.clip(), c[t](a, S(e, n)), a.restore();
			});
		}
	};
}
var F = Array.from({ length: 21 }, (e, t) => A(k.slice(0, t)));
function I(e, t) {
	e.fillStyle = G(e, 235, 195), X(e, .025, .03, .95, .77, .035), e.fill();
	let n = .6425;
	e.fillStyle = W(5), e.fillRect(.6255, .03, .034, .77), J(e, n, .13, .055, W(5)), J(e, n, .13, .033, N);
	let r = 0, i = 0;
	for (let n = 0; n < 20; n++) {
		let a = .5 + n * .35;
		if (t < a) continue;
		let o = v(t, a, a + .65), s = k[n] === "1", c = s ? _(.09, .5774999999999999, w(n, 91)) : _(.7075, .92, w(n, 91)), l = _(.24, .72, w(n, 412)), u = (1 - y(o)) * .29;
		J(e, c, l - u, .042, W(5)), s || J(e, c, l - u, .026, W(250)), o === 1 && (r++, i = a + .65);
	}
	let a = F[Math.max(0, r - 1)], o = F[r], s = y(v(t, i, i + .22)), c = _(a[0], o[0], s), l = _(a[1], o[1], s);
	e.fillStyle = W(90), e.fillRect(.025, .868, .95, .047), e.fillStyle = W(245), e.fillRect(.025 + c * .95, .847, (l - c) * .95, .09), e.fillStyle = N, e.fillRect(.6305, .831, .024, .123);
}
var L = [
	6,
	7,
...
function z(e, t) {
	let n = t >= 4.35 && t < 5.15, r = (t < 4.35 ? _(8, 23, v(t, .65, 4.35)) : t < 5.15 ? _(23, 13, y(v(t, 4.35, 5.15))) : _(13, 24, v(t, 5.15, 7.8))) - 8, i = .125, a = g(r - 3.3, 0, 9.8) * i;
	e.fillStyle = G(e, 245, 200), e.fillRect(0, 0, 1, 1), e.save(), e.translate(0, -a);
	let o = [
		.072,
		.06,
		.06,
	...
	if (o.forEach((t, n) => {
		e.fillStyle = W(5), e.fillRect(s, 0, .016, 2.2);
		for (let r = 0; r < 17; r++) R[r] === L[n] && e.fillRect(s + .022, .075 + r * i, t - .027, .072);
		s += t;
	}), e.fillStyle = W(5), [
		.04,
		.665,
		...
		let t = .15 + r * i;
		e.fillStyle = N, e.fillRect(.155, t, .8, .045), e.fillStyle = K(e, .165, t + .01, .78, .025), e.fillRect(.165, t + .01, .78, .025);
	}
	if (e.restore(), e.beginPath(), e.moveTo(.13, .16), e.bezierCurveTo(.065, .16, .12, .44, .035, .5), e.bezierCurveTo(.12, .56, .065, .84, .13, .84), e.lineWidth = .033, e.strokeStyle = W(5), e.stroke(), n && J(e, .085, _(.84, .16, v(t, 4.35, 5.15)), .042, N), t >= 7.8) {
		let t = 2.075 - a;
		e.fillStyle = N, e.fillRect(.799, t, .078, .072), e.fillStyle = K(e, .809, t + .01, .058, .052), e.fillRect(.809, t + .01, .058, .052);
	}
}
function B(e, t) {
	let n = E.length, r = v(t, .6, 8.3) * n, i = Math.min(n, Math.floor(r)), a = i === n ? 0 : r - i, o = D(i + +(a > .22)), s = i + y((a - .25) / .75);
	e.save(), e.translate(.5, .5), e.rotate(-.16), e.translate(-.5, -.5), e.fillStyle = G(e, 245, 190), e.fillRect(-.3, .39, 1.6, .5);
	let c = .245;
	for (let t = -3; t <= 13; t++) {
		let n = .64 + (t - s) * c;
		e.fillStyle = W(5), e.fillRect(n - c / 2, .39, .024, .5);
		let r = o.tape[t];
		r === 0 ? (X(e, n - .063, .49, .126, .29, .05), e.lineWidth = .032, e.strokeStyle = W(5), e.stroke()) : r === 1 && Y(e, [
			[n - .05, .54],
			[n + .008, .49],
			[n + .008, .78]
		], .043, W(5));
	}
	let l = i < n && E[i] !== null ? Math.sin(Math.PI * v(a, 0, .3)) * .042 : 0;
	e.fillStyle = W(220), X(e, .465, .055 + l, .35, .26, .035), e.fill(), e.fillStyle = W(5), e.fillRect(.515, .09 + l, .25, .12), e.fillStyle = N, e.fillRect(.615, .26 + l, .05, .135), e.restore();
}
var V = Array.from({ length: 161 }, (e, t) => [.045 + t / 160 * .91, .91 - T(t / 160) * .76]);
function te(e, t) {
	let n = _(.025, .975, v(t, .65, 8.3)), r = .045 + n * .91, i = .91 - T(n) * .76;
	e.beginPath(), e.moveTo(.045, .91);
	for (let [t, n] of V) e.lineTo(t, n);
	e.closePath(), e.fillStyle = G(e, 245, 180), e.fill(), Y(e, [
		[.025, .08],
		[.025, .935],
		[.98, .935]
	], .022, W(180)), e.fillStyle = W(5), e.fillRect(r - .021, i, .042, .91 - i), q(e, r, i, .058);
}
function ne(e, t) {
	let n = _(-.1, .61, v(t, .65, 8.3));
	for (let t = 0; t < 10; t++) for (let r = 0; r < 8; r++) {
		let i = .02 + r * .066, a = .06 + t * .086;
		e.fillStyle = W(_(235, 5, 1 - y((Math.abs(i + .024 - (n + (a - .5) * .48)) - .055) / .035))), e.fillRect(i, a, .048, .065);
	}
	let r = (e) => {
		let t = n + (e - .5) * .48;
		return y((t + .1) / .16) * (1 - y((t - .44) / .16));
	};
	for (let n = 0; n < 4; n++) {
		let i = .17 + n * .215, a = [
			0,
		...
		Y(e, [[.54, i], [.65, i]], .032, W(175)), Y(e, [[.69, i], [.81, .26 + a * .24]], .027, W(175)), J(e, .66, i, .047, W(80 + r(i) * 165));
		let o = (Math.max(0, t - .7) / 2.4 + n * .14) % 1;
		t > .7 && J(e, o < .42 ? _(.54, .66, o / .42) : _(.66, .83, (o - .42) / .58), o < .42 ? i : _(i, .26 + a * .24, (o - .42) / .58), .023, N);
	}
	for (let t = 0; t < 3; t++) {
		let n = .26 + t * .24;
		Y(e, [[.85, n], [.955, .5]], .03, W(190)), J(e, .83, n, .059, W(95 + r(n) * 150));
	}
	J(e, .96, .5, .065, W(235)), J(e, .96, .5, .035 * r(.5), W(5));
}
var H = Array.from({ length: 15 }, (e, t) => {
	let n = Math.floor(Math.log2(t + 1)), r = t - (2 ** n - 1);
	return [.83 - n * .26, .02 + (r + .5) / 2 ** n * .96];
});
function re(e, t, n) {
	let r = .65, i = 7.45, a = 6.8 / n.events.length, o = y(v(t, n.events.at(-1)?.kind === "cutoff" ? i - a : i, 8)), s = n.events.flatMap((e, n) => e.kind === "cutoff" ? [{
		...e,
		remaining: 1 - y(v(t, r + n * a, r + (n + 1) * a))
	}] : []), c = H.slice(1).map((e, t) => {
		let r = t + 1, i = H[Math.floor((r - 1) / 2)], a = (i[0] + e[0]) / 2, c = [
			i,
			[a, i[1]],
			[a, e[1]],
			e
		...
		if (u) {
			let e = H[u.node];
			c = c.map(([t, n]) => [_(e[0], t, u.remaining), _(e[1], n, u.remaining)]), l *= u.remaining;
		}
		return {
			path: c,
			width: l
	...
	c.forEach(({ path: t, width: n }) => {
		n > 0 && Y(e, t.map(([e, t]) => [e + .014, t + .021]), n + .012, W(5));
	}), c.forEach(({ path: t, width: n }) => {
		n <= 0 || (Y(e, t, n, W(115)), Y(e, t.map(([e, t]) => [e, t - n * .1]), n * .72, W(235)));
	}), e.fillStyle = G(e, 235, 190), X(e, .65, .29, .39, .43, .025), e.fill(), Z(e, [
		[.14, .88],
		[.88, .88],
		[.8, .74],
		[.7, .67],
		...
		[.46, .59],
		[.3, .75]
	].map(([e, t]) => [.69 + e * .3, .34 + t * .34]), W(5)), J(e, .846, .34 + .28 * .34, .011, W(230));
	let l = H[7][1];
	n.events.forEach((e, n) => {
		e.kind === "evaluate" && (l = _(l, H[e.node][1], y(v(t, r + n * a, r + (n + 1) * a))));
	});
	let u = n.path.at(-1);
	l = _(l, H[u][1], o);
	let d = .054 * y(v(t, r, r + a));
	q(e, H[u][0], l, d);
}
...
function ae(e, t) {
	let n = v(t, .65, 8.3) * 3, r = Math.min(2, Math.floor(n)), i = n - r, { symmetric: a, broken: o, before: s, after: c } = ie[r], l = y(v(i, .45, .64)), u = [];
	e.fillStyle = G(e, 235, 195), e.fillRect(0, 0, 1, 1);
	for (let t = 0; t < 6; t++) {
		let n = .25 + t * .124;
		for (let a of [-1, 1]) {
	...
	for (let t = 0; t < 2; t++) {
		...
		Y(e, [
			...
			[t === 0 ? .04 : .96, .09],
			[.5, .09]
		], .028, W(5)), J(e, n, .56, .085, W(5)), J(e, n, .56, .021 + _(s.hidden[t], c.hidden[t], l) * .047, W(245));
	}
	for (let t = 0; t < 6; t++) {
		J(e, .5, .25 + t * .124, .066, W(5));
		let n = _(a[t], o[t], l);
		J(e, .5, .25 + t * .124, .046 * n, W(245));
	}
	[
		.34,
		.46,
		...
		e.fillStyle = W(5), e.fillRect(t, .55, .08, .025);
	}), J(e, .5, .09, .086, W(5)), q(e, .5, .09, .016 + _(s.output, c.output, l) * .062), u.forEach(([t, n]) => q(e, t, n, .027));
}
var oe = [
	0,
	3,
...
function ce(e, t) {
	let n = v(t, .65, 8.3), r = "rgb(235,75,75)";
	e.save(), e.translate(.5, .5), e.rotate(-.12), e.scale(1, .88), e.translate(-.5, -.5);
	let i = _(.092, .105, n), a = ([e, t]) => [.5 + (e - _(10.4, 11, n)) * i, .5 + (t - _(13.8, 14.3, n)) * i];
	e.fillStyle = G(e, 245, 200), e.fillRect(-.5, -1.2, 2, 2.6);
	for (let t = 0; t < 19; t++) Y(e, [a([t, 0]), a([t, 18])], .014, W(5)), Y(e, [a([0, t]), a([18, t])], .014, W(5));
	for (let t of [
		3,
		9,
		15
	...
	let d = (t, n, r = 0) => {
		let [o, s] = a(t);
		J(e, o + .008 + r * .15, s + .012, i * .54, W(70)), J(e, o, s - r, i * .5, W(5)), n && J(e, o - .002, s - r - .004, i * .35, W(250));
	};
	s.forEach((e) => d(e, !0)), o.forEach((e) => d(e, !1)), t >= .95 && t < 4.7 && (e.save(), e.globalAlpha *= 1 - y(v(t, 4.4, 4.7)), u.forEach(({ point: n, frequency: o }, s) => {
...
var le = [
	...
	let n = t / 80, r = Q(Math.max(0, n - .001)), i = Q(Math.min(1, n + .001));
	return {
		center: Q(n),
		tangent: i.map((e, t) => e - r[t]),
		halfWidth: _(.5, .24, y(n))
	};
});
function de(e, t) {
	let n = v(t, .65, 8.3);
	le.forEach(({ points: [t, r, i, a], width: o }) => {
		fe(e, [
			t,
			[_(.34, r[0], n), _(.31, r[1], n)],
			[_(.49, i[0], n), _(.65, i[1], n)],
			a
		], o);
	}), Z(e, [
		[.115, .085],
...
function fe(e, t, n) {
	...
	U(e, r, 0, 4, 5, .025, .02), [
		...
		205,
		235
	].forEach((t, n) => U(e, r, n, n + 1, t));
}
function U(e, t, n, r, i, a = 0, o = 0) {
	e.beginPath();
	let s = n / 2 - 1, c = r / 2 - 1;
	...
	e.closePath(), e.fillStyle = W(i), e.fill();
}
function pe(e) {
	...
	for (let e = 0; e < r.data.length; e += 4) {
		let t = g((r.data[e] * .2126 + r.data[e + 1] * .7152 + r.data[e + 2] * .0722 - 55) * 1.95, 0, 255);
...
function G(e, t, n) {
	...
	return r.addColorStop(0, W(t)), r.addColorStop(1, W(n)), r;
}
...
const float REVEAL_STARTS[9] = float[9](${p.map((e) => e.toFixed(6)).join(", ")});
${ge}
...
void rosette(float vertex) {
  ...
  float cycle = u_time * 6.28318530718 / ${d};
...
void main() {
  ...
  float arrival = REVEAL_STARTS[int(tile)] + ${m} * seed;
...
`, $ = {
	...
	constructor(e, t, r = n) {
		...
		this.gl = i, this.atlas = P(t, r);
		try {
			if (this.points = Ce(i, _e, ve), this.masks = Ce(i, ye, be), this.texture = i.createTexture(), !this.texture) throw Error("Unable to allocate the artwork texture.");
	...
	render(e, t = $) {
		...
		if (n.bindTexture(n.TEXTURE_2D, this.texture), r !== this.lastTime) {
			this.atlas.draw(r), n.texSubImage2D(n.TEXTURE_2D, 0, 0, 0, n.RGBA, n.UNSIGNED_BYTE, this.atlas.canvas), this.phase = x(r);
			for (let e = 0; e < this.rosettes.length; e++) this.rosettes[e] = C(r, e);
			this.lastTime = r;
		}
		let i = t.masks ? this.masks : this.points;
		n.useProgram(i.handle);
		let a = i.uniforms;
		if (n.uniform1i(a.u_atlas, 0), n.uniform2f(a.u_resolution, this.canvas.width, this.canvas.height), n.uniform1f(a.u_focus, t.focus), n.uniform1f(a.u_crop, Number(t.crop)), n.clear(n.COLOR_BUFFER_BIT), t.masks) n.drawArrays(n.TRIANGLES, 0, 3);
		else {
			n.uniform1f(a.u_time, this.phase), n.uniform4fv(a.u_rosettes, this.rosettes), n.uniform1f(a.u_grid, t.grid), n.uniform1f(a.u_size, t.size), n.uniform1f(a.u_jitter, t.jitter), n.uniform1f(a.u_reveal, Number(t.reveal)), n.uniform1f(a.u_imperfection, t.imperfection), n.uniform1f(a.u_palette_delay, t.paletteDelay * f), n.uniform1f(a.u_red_advance, t.redAdvance * f), n.uniform1f(a.u_colour_contrast, Number(t.colourContrast)), n.uniform1f(a.u_red_restraint, t.redRestraint), n.uniform1f(a.u_colour_waves, Number(t.colourWaves)), n.uniform1f(a.u_micro_jitter, t.microJitter), n.uniform1f(a.u_soft_red, Number(t.softRed));
			let e = t.grid ** 2;
...
function Ce(e, t, n) {
	...
	try {
		...
		return {
			handle: r,
			uniforms: Object.fromEntries(Se.map((t) => [t, e.getUniformLocation(r, t)]))
		};
	} catch (t) {
...
function Te(n, { assets: r = new URL(
	/* @vite-ignore */
	".",
	import.meta.url
), focus: i = -1, crop: a = !0, autoplay: o = !0, stillTime: s = e } = {}) {
	let c = n.querySelector("[data-motifs-canvas]"), l = n.querySelector("[data-motifs-poster]"), u = n.querySelector("[data-motifs-toggle]"), d = matchMedia("(prefers-reduced-motion: reduce)"), f = new AbortController(), p = { signal: f.signal }, ee = {
		...$,
		...t,
		focus: i,
		crop: a
	}, m = new Image(), h = a ? c : document.createElement("canvas"), g = null, _ = null, v = null, y = o && !d.matches ? 0 : Math.round(s * 30), b = Math.floor(y), x = null, S = 0, C = o && !d.matches, w = !1, T = !1, E = !1, D = !1, O = !1, k = matchMedia(`(resolution: ${devicePixelRatio}dppx)`);
	u?.addEventListener("click", () => {
		C = !C, M();
	}, p), d.addEventListener("change", () => {
		d.matches && (C = !1, y = Math.round(s * 30), b = Math.floor(y), I()), M();
	}, p), document.addEventListener("visibilitychange", R, p), window.addEventListener("resize", F, p), window.addEventListener("pagehide", (e) => {
		e.persisted ? (T = !0, M()) : V();
	}, p), window.addEventListener("pageshow", (e) => {
		e.persisted && (T = !1, R());
	}, p), h.addEventListener("webglcontextlost", (e) => {
		e.preventDefault(), B(/* @__PURE__ */ Error("Graphics context lost."));
	}, p), h.addEventListener("webglcontextrestored", () => {
		E || (O = !1, N(!0));
	}, p), k.addEventListener("change", z, p);
	let A = new ResizeObserver(F);
	A.observe(n);
	let j = new IntersectionObserver((e) => {
		w = e[e.length - 1].isIntersecting, R();
	});
	return j.observe(n), n.dataset.renderer = C ? "loading" : "poster", l.hidden = C, n.dataset.frame = String(b), M(), V;
	function M() {
		cancelAnimationFrame(S), S = 0, x = null, !C && n.dataset.renderer === "loading" && (y = Math.round(s * 30), b = Math.floor(y), n.dataset.frame = String(b), n.dataset.renderer = "poster", l.hidden = !1);
		let e = !!g && C && L();
		n.dataset.playing = String(e), u && (u.hidden = O || E, u.textContent = C ? "Pause animation" : "Play animation"), e ? S = requestAnimationFrame(P) : !g && !O && C && L() && N();
	}
	async function N(e = !1) {
		if (!(D || E || g)) {
			D = !0;
			try {
				if (m.src ||= new URL("alexnet-filters.png", r).href, await m.decode(), E || O || !e && (!C || !L())) return;
				if (h !== c) {
					if (_ = c.getContext("2d", { alpha: !1 }), !_) throw Error("Canvas 2D is unavailable.");
					v = we();
				}
				g = new xe(h, m), F();
			} catch (e) {
	...
	function P(e) {
		...
		x !== null && (y = (y + (e - x) * 30 / 1e3) % 600), x = e;
		let t = Math.floor(y);
		t !== b && (b = t, I()), g && (S = requestAnimationFrame(P));
	}
	function F() {
		if (!g || !L()) return;
		c.hidden = !1;
		let e = c.getBoundingClientRect(), t = Math.min(1600, Math.max(1, Math.round(Math.max(e.width, e.height) * Math.min(devicePixelRatio, 2))));
		_ && (c.width !== t || c.height !== t) && (c.width = t, c.height = t), g.resize(Math.max(1200, t), Math.max(1200, t), 1), I();
	}
	function I() {
		if (g && !E && L()) try {
			g.render(b / 30, ee), _ && v && v(h, _), l.hidden = !0, n.dataset.renderer = "webgl2", delete n.dataset.error, n.dataset.frame = String(b);
		} catch (e) {
			B(e);
		}
	}
	function L() {
		return w && !document.hidden && !T && !E;
	}
	function R() {
		F(), M();
	}
	function z() {
		k.removeEventListener("change", z), k = matchMedia(`(resolution: ${devicePixelRatio}dppx)`), k.addEventListener("change", z, p), F();
	}
	function B(e) {
		g?.dispose(), g = null, _ = null, v = null, O = !0, c.hidden = !0, l.hidden = !1, n.dataset.renderer = "fallback", n.dataset.error = e instanceof Error ? e.message : "Animation unavailable.", M();
	}
	function V() {
		E || (E = !0, C = !1, M(), f.abort(), A.disconnect(), j.disconnect(), g?.dispose(), g = null, _ = null, v = null, h.width = h.height = 1, c.hidden = !0, c.width = c.height = 1, l.hidden = !1, n.dataset.renderer = "poster");
	}
}
//#endregion
export { e as HOMEPAGE_STILL_TIME, Te as mountMotifs };
